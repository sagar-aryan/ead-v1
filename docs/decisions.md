...
| Unmeasurable feature | Cycle distance is dropped when ZUPT quality is below 0.15 (was: exactly 0, changed 2026-09-18, PROB-015) | Doc 06 §3 says missing features leave the denominator and reduce confidence, without saying which can be missing. Distance is the only feature that depends on a zero-velocity window. |
